import os
import uuid
from fastapi import APIRouter, UploadFile, File, HTTPException
from azure.storage.blob import BlobServiceClient
from azure.identity import DefaultAzureCredential

router = APIRouter(
    prefix="/storage",
    tags=["storage"]
)

# Configuration from your automation template
ACCOUNT_URL = "https://codesipsdocs2026.blob.core.windows.net"
CONTAINER_NAME = "agent-docs"

def get_blob_service_client():
    # Week 1 Goal: Using Managed Identity (DefaultAzureCredential)
    return BlobServiceClient(ACCOUNT_URL, credential=DefaultAzureCredential())

@router.post("/upload")
async def upload_document(file: UploadFile = File(...)):
    if file.content_type != "application/pdf":
        raise HTTPException(status_code=400, detail="Only PDFs allowed")
    
    unique_filename = f"{uuid.uuid4()}-{file.filename}"
    service_client = get_blob_service_client()
    
    try:
        blob_client = service_client.get_blob_client(container=CONTAINER_NAME, blob=unique_filename)
        contents = await file.read()
        blob_client.upload_blob(contents, overwrite=True)
        return {"filename": unique_filename, "status": "stored", "container": CONTAINER_NAME}
    except Exception as e:
        # Catching the RBAC error or missing container error
        raise HTTPException(status_code=500, detail=f"Storage Error: {str(e)}")

@router.get("/list")
async def list_documents():
    service_client = get_blob_service_client()
    container_client = service_client.get_container_client(CONTAINER_NAME)
    
    blobs = container_client.list_blobs()
    return [{"name": b.name, "size": b.size} for b in blobs]